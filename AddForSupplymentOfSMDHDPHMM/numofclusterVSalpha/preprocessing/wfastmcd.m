...
%   $Revision: 1.1.6.3 $ $Date: 2008/08/22 20:37:05 $ 
...
if n==0
   error('Wavelet:FunctionArgVal:Invalid_ArgVal', ...
       'All observations have missing or infinite values.')
elseif n > nmax
   error('Wavelet:FunctionArgVal:Invalid_ArgVal', ...
       'The program allows for at most %s  observations.',int2str(nmax))
end

if p > pmax
   error('Wavelet:FunctionArgVal:Invalid_ArgVal', ...
       'The program allows for at most %s variables.',int2str(pmax))
end

if n < 2*p
   error('Wavelet:FunctionArgVal:Invalid_ArgVal', ...
       'Need at least 2*(number of variables) observations.')
end
...
if h < hmin
    error('Wavelet:FunctionArgVal:Invalid_ArgVal', ...
        'The MCD must cover at least %s observations.',int2str(hmin))
elseif h > n
    msg = ['quan is greater than the number ' ... 
           'of non-missings and non-infinites.'];
    error('Wavelet:FunctionArgVal:Invalid_ArgVal',msg)
end
      ...
      for i = 1:nsamp
          ...
          elseif fine
              ...
              if det(covmat) < exp(-50*p)
                  covmat  = cov(data(index,:))+eps*eye(size(covmat,1));     %Added by Guodong Tian on 7.25.2011
                  % eigvct : contains the coefficients of the hyperplane.
%                   eigvct = eigs(covmat,1,0,struct('disp',0));
                  eigvct = eigs(covmat,1,0,struct('disp',0,'issym',1));      %modified by Guodong Tian on 8.29.2011
                  if ~part , nbONES = n; else nbONES = group(k); end
                  onesIDX = ones(1,nbONES);
%                   dist = ...
%                     abs(sum((data - meanvct(onesIDX,:))'.*eigvct(:,onesIDX)));
                  dist = ...
                    abs(sum((data - meanvct(onesIDX,:))'.*eigvct));      %modified by Guodong Tian on 7.25.2011
