function theta = sample_theta1(theta,Ustats,obsModel)
...
switch obsModel.type
    ...
    case 'Multinomial'
        ...
        Kz = size(store_counts,2);
        Ks = size(store_counts,3);
        for kz=1:Kz
            for ks = 1:Ks
%                 p(kz,ks,:) = randdirichlet([alpha_vec'+store_counts(:,kz,ks)])';
                p(kz,ks,:) = alpha_vec+store_counts(:,kz,ks)';
                p(kz,ks,:) = p(kz,ks,:) / sum(p(kz,ks,:));
            end
        end
...
function theta = sample_theta_submodule(theta,Ustats,priorType,prior_params)
...
switch priorType
    ...
    case {'N-IW-N','Afixed-IW-N','ARD'}
        ...
        elseif strcmp(priorType,'ARD')
            ...
            if ~sum(sum(sum(theta.ARDhypers)))
                %                         for kz=1:Kz
                %                             for ks=1:Ks
                %                                 aa = zeros(1,numHypers);
                %                                 for ii=1:numHypers;
                %                                     aa(ii) = randgamma(a_ARD);
                %                                 end
                %                                 ARDhypers(:,kz,ks) =  aa ./ b_ARD;
                %                             end
                %                         end
                ARDhypers = ones(numHypers,Kz,Ks);
            else
                ARDhypers = theta.ARDhypers;
            end
