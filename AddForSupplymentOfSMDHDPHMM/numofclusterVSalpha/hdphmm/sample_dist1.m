...
function dist_struct = sample_dist1(stateCounts,hyperparams,model)
...
%             beta_c(:,ii) = randdirichlet((sum(barM0(:,:,ii),1)+gamma0*beta_vec)');
            beta_c(:,ii) = (sum(barM0(:,:,ii),1)+gamma0*beta_vec)';
            beta_c(:,ii) = beta_c(:,ii) ./ sum(beta_c(:,ii));
        end
    case 'finite'
...
%         pi_z(jj,:,ii) = randdirichlet((alpha0*beta_c(:,ii)' + kappa_vec + N(jj,:,ii))')';        
        pi_z(jj,:,ii) = alpha0*beta_c(:,ii)' + kappa_vec + N(jj,:,ii);        
        pi_z(jj,:,ii) = pi_z(jj,:,ii) ./ sum(pi_z(jj,:,ii));
    end
end
for jj = 1:Kz
...
%     pi_init(:,ii) = randdirichlet(alpha0*beta_c(:,ii) + N(Kz+1,:,ii)');
    pi_init(:,ii) = alpha0*beta_c(:,ii) + N(Kz+1,:,ii)';
    pi_init(:,ii) = pi_init(:,ii) ./ sum(pi_init(:,ii));
end
