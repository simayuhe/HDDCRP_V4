function [fwd_msg, neglog_c] = forward_message_vec(likelihood,loglike_normalizer,blockEnd,pi_z,pi_s,pi_init)
...
fwd_msg(:,1) = fwd_msg(:,1) / (sum_fwd_msg+eps);
% Add the constant from normalizing the forward message:
neglog_c(1) = neglog_c(1)+log(sum_fwd_msg+eps);
...
for tt = 1:T-1
  ...
  fwd_msg(:,tt+1) = fwd_msg(:,tt+1) / (eps+sum_fwd_msg);
  
  % Add the constant from normalizing the forward message:
  neglog_c(tt+1) = neglog_c(tt+1)+log(sum_fwd_msg+eps);
end