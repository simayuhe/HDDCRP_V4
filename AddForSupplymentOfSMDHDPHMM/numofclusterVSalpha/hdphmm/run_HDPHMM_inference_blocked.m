function run_HDPHMM_inference_blocked(saveDir,trial_vec,codes,blockSize,K,Kc,Kz,Ks,init_c,init_z,num_iter,resample_kappa,varargin)
	change_model = 1;
    sample_c = 1;
	if ~isempty(varargin)
...
for jj = 1:J
    ...
    data_struct(jj).blockSize = blockSize{jj};
...
for t=trial_vec
    ...
    if change_model
        HDPHMMDPinference_warmup(data_struct,model,settings);
    else
