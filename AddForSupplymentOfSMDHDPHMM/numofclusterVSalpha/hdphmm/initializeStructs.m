function [theta, Ustats, stateCounts, stateSeq, INDS, hyperparams, data_struct, model, S] = initializeStructs(model,data_struct,settings)
...
switch model.obsModel.type
    ...
    case 'Gaussian'
        ...
        for ii=1:length(data_struct)
            if isempty(data_struct(ii).blockSize) || ~isfield(data_struct(1),'blockSize')
...
stateCounts.N = zeros(Kz+1,Kz,Kc);
stateCounts.Ns = zeros(Kz,Ks);
stateCounts.uniqueS = zeros(1,Kz);
stateCounts.M0 = zeros(Kz+1,Kz,Kc);
stateCounts.barM0 = zeros(Kz+1,Kz,Kc);
stateCounts.sum_w = zeros(Kz,Kc);
stateCounts.M = zeros(Kz,Kc);
stateCounts.L = zeros(1,Kc);
...
if strcmp(model.obsModel.type,'SLDS')
    ...
    for ii = 1:length(data_struct)
        ...
        S.stateSeq(1:numStateSeqSaves,ii) = struct('z',zeros(1,T),'s',zeros(1,data_struct(ii).blockEnd(end)));
    end
    S.dist_struct(1:numSaves)...
        = struct('pi_z',zeros(Kz,Kz,Kc),'pi_init',zeros(Kz,Kc),'pi_s',zeros(Kz,Ks),...
        'pi_c',zeros(1,Kc),'beta_c',zeros(Kz,Kc),'beta_vec',zeros(1,Kz));
end
S.cluster = zeros(numSaves,length(data_struct));
