...
raw_data_file = './data/CASIA/CASIA_tjc.mat';%原始数据集
...
for myi=1:1:length(myiter)
    for myj=1:3
        ...
        num_labs = 1;
        %从头处理，包括滤波，单词本制作，以及轨迹编码过程都放在这里进行处理了
        
        do_pre_process = 0;%选择是否进行之前的处理，了解一下哪些是要每次都做的哪些是做一次就够的
        do_denoise = 0;
        do_med_flt = 0;
        med_width = 5;
        do_resample = 0;
        ...
        Kc = 20;%*ones(1,num_labs);%文档类的初始个数
